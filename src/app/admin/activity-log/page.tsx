...
export default function ActivityLogPage() {
  ...
  const groupedLogsByMonth = Object.entries(groupedLogs).reduce(
    (acc, [date, logs]) => {
    ...
    {} as Record<string, Record<string, typeof groupedLogs[string]>>
  );
    ...
    <AdminDashboardWrapper requireModule="activity-log">
      <div className="space-y-6">
        ...
        {isLoading ? (
          ...
          // </div>
          <div className="space-y-8">
            {Object.entries(groupedLogsByMonth).map(([month, dates]) => (
              <div
                key={month}
                className="rounded-xl border border-white/10 overflow-hidden pt-1"
              >
                {/* Month Header */}
                <button
                  onClick={() => toggleMonth(month)}
                  className="w-full flex items-center justify-between px-6 py-4 bg-white/[0.03] hover:bg-white/[0.05] transition"
                >
                  <div className="font-semibold text-white tracking-wider">
                    {month}
                  </div>

                  <svg
                    className={`w-5 h-5 transition-transform ${expandedMonths[month] ? "rotate-180" : ""
                      }`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                            ...
                            <div key={idx} className="bg-white/[0.02] border border-white/[0.05] rounded-xl p-3">
                              ...
                              <div className="flex items-center gap-3 text-xs">
                                {change?.oldVal !== undefined && change?.oldVal !== null ? (
                                  <span className="text-white/50 line-through truncate">
                                    {/*  max-w-[120px] */}
...
// // ---- NEW: shared value formatter used by the Data Changes block ----
// function formatDisplayValue(val: any): string | null {
...
//                 // Explicit oldData/newData schema present
//                 if (oldData !== undefined || newData !== undefined) {
//                   const isCreation = oldData === null || oldData === undefined;

//                   // ---- CREATION: no diff arrows, clean key-value grid ----
//                   if (isCreation) {
//                     const entries = Object.entries(newData || {})
//                       .map(([key, val]) => ({ key, display: formatDisplayValue(val) }))
...
//                     return (
//                       <div className="md:col-span-2 lg:col-span-3 pt-4 border-t border-white/[0.08] mt-2 space-y-4">
//                         <p className="text-white/80 text-xs tracking-wider font-semibold flex items-center gap-2">
...
//                               </p>
//                               <div
//                                 className="text-xs text-emerald-400/90 font-medium truncate"
//                                 title={display ?? ""}
//                               >
//                                 {display}
//                               </div>
...
//                   // ---- UPDATE: diff arrows, only real / changed fields ----
//                   const safeOldData = oldData || {};
//                   const safeNewData = newData || {};
//                   const allKeys = Array.from(new Set([...Object.keys(safeOldData), ...Object.keys(safeNewData)]));
...
//                     .map((key) => {
//                       const oldValRaw = safeOldData[key];
//                       const newValRaw = safeNewData[key];
//                       if (JSON.stringify(oldValRaw) === JSON.stringify(newValRaw)) return null;
//                       return {
//                         key,
//                         oldVal: formatDisplayValue(oldValRaw),
//                         newVal: formatDisplayValue(newValRaw),
//                       };
//                     })
//                     .filter(Boolean) as { key: string; oldVal: string | null; newVal: string | null }[];
...
//                               <div className="flex items-center gap-3 text-xs">
//                                 {change.oldVal ? (
//                                   <span className="text-white/50 line-through truncate max-w-[120px]" title={change.oldVal}>
//                                     {change.oldVal}
//                                   </span>
//                                 ) : (
//                                   <span className="text-white/30 italic">None</span>
...
//                                 {change.newVal ? (
//                                   <span className="text-emerald-400/90 font-medium truncate max-w-[120px]" title={change.newVal}>
//                                     {change.newVal}
//                                   </span>
//                                 ) : (
//                                   <span className="text-white/30 italic">None</span>
...
//                   // No real changes detected between oldData and newData
//                   return null;
//                 }

//                 // Fallback: no explicit oldData/newData schema, show raw payload
//                 if (Object.keys(rawData).length > 0) {
//                   return (
//                     <div className="md:col-span-2 lg:col-span-3 pt-4 border-t border-white/[0.08] mt-2">
//                       <span className="block text-white/40 text-xs uppercase tracking-wider font-semibold mb-3">Raw Payload</span>
