...
// Merge saved permissions JSON into our local state (handles missing keys gracefully)
const mergePerms = (saved: Record<string, any>): PermissionsState => {
    const base = buildEmptyPerms();
    for (const mod of MODULES) {
        if (saved[mod.key]) {
            for (const perm of mod.perms) {
                base[mod.key][perm] = !!saved[mod.key][perm];
            }
        }
    }
    return base;
...
export default function EditRolePage() {
    ...
    // ── Extract roleId from URL on mount, then load role ────────────────────────
    ...
    useEffect(() => {
        ...
        const loadRole = async () => {
            try {
                ...
                if (res.ok && data.role) {
                    // if (data.role.name === "Super Admin" || data.role.name === "SuperAdmin") {
                    //     toast.error("You cannot edit the Super Admin role.");
                    //     router.push("/admin/roles");
                    //     return;
                    // }
                    setRoleName(data.role.name || "");
                    try {
                        const parsed = JSON.parse(data.role.permissions || "{}");
                        // Handle double-stringified permissions
                        const obj = typeof parsed === "string" ? JSON.parse(parsed) : parsed;
                        setPermissions(mergePerms(obj));
                    } catch {
                        setPermissions(buildEmptyPerms());
                    }
                } else {
                    setFormError(data.error || "Failed to load role.");
                }
            } catch {
                setFormError("Failed to load role data.");
            } finally {
                setPageLoading(false);
            }
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setFormLoading(true);
        try {
            const res = await adminFetch(`/api/admin/roles/${roleId}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    name: formattedName,
                    permissions: JSON.stringify(permissions), // Ensure stringified
                }),
            });
            const data = await res.json();
            if (res.ok) {
                ...
                toast.error(data.error || "Failed to update role. Please try again.");
        ...
        return (
            <AdminDashboardWrapper>
                <div className="space-y-6 animate-pulse">
                    <div className="h-9 bg-white/[0.05] rounded-xl w-48" />
                    ...
                    <div className="rounded-2xl bg-gradient-to-br from-[#141414] to-[#0f0f0f] border border-white/[0.08] p-6 space-y-4">
                        {[...Array(4)].map((_, i) => (
        ...
        return (
            <AdminDashboardWrapper>
                <div className="flex flex-col items-center justify-center py-32 text-center">
                    ...
                    <p className="text-white/40 text-sm mb-6">Role not found</p>
                    <button onClick={() => router.push("/admin/roles")} className="px-5 py-2.5 rounded-xl bg-white/[0.06] hover:bg-white/[0.1] text-white/70 text-sm transition-all">
                        Back to Roles
                    </button>
                </div>
            </AdminDashboardWrapper>
    ...
    return (
        <AdminDashboardWrapper requireModule="roles" requireAction="edit">
