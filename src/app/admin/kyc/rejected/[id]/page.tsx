// app/admin/kyc/rejected/[id]/page.tsx
...
function RejectedKYCDetailsContent() {
    ...
    useEffect(() => {
        ...
        const fetchKycData = async () => {
            try {
                const res = await adminFetch(`/api/admin/kyc/rejected/${id}`);
        ...
        <>
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <h1 className="text-2xl font-bold text-white">Rejected KYC Details</h1>
                </div>

                {/* Tabs for Enterprise Accounts */}
                {kycData.accountType?.toLowerCase() === "enterprise" && (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {/* Left Column: ID Verification Section */}
                    <div className="space-y-6">
                                                            ...
                                                            <div>
                                                                ...
                                                                <p className="text-white/90 text-sm font-medium tracking-wide">
                                                                    {kycData?.representativeName ||
                                                                        (idData?.first_name || idData?.last_name ?
                                                ...
                                                <div key={index} className="bg-[#141414] border border-white/[0.08] rounded-2xl overflow-hidden">
                                                    <div className="bg-gradient-to-r from-white/[0.05] to-white/[0.02] p-4 border-b border-white/[0.08] flex items-center justify-between">
                                                        ...
                                                        {/* <div className={`flex items-center gap-2 px-3.5 py-1.5 rounded-full border shadow-sm transition-all duration-300 ${doc.verificationStatus === 'success' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' :
                                                            ...
                                                            <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${doc.verificationStatus === 'success' ? 'bg-emerald-500' :
                                                                doc.verificationStatus === 'failed' ? 'bg-red-500' :
                                                                    'bg-amber-500'
                                                                }`}></div>
                                                            <span className="text-[10px] font-bold tracking-widest capitalize">
                                                    ...
                                                    <div className="p-6 space-y-8">
                                                                    ...
                                                                    {photoSrc && (
                                                                        <div className="lg:col-span-1 flex items-start justify-center">
                                                                            <div className="relative w-[160px] h-[200px] rounded-lg overflow-hidden border-2 border-white/[0.1] bg-black/20 shadow-xl group">
                                                                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                                                                <InnerZoomImage src={photoSrc} alt="Document Photo" className="w-full h-full" />
                                                                            </div>
                                                                        </div>
                                                                    )}
                                                                </div>
                                                                ...
                                                                <div className="!mt-0">
                                                                    {Object.entries(extractedData)
                                                                        ...
                                                                        .map(([key, value]) => {
                                                                            ...
                                                                            if (!lowerKey.includes('address') && !lowerKey.includes('care_of')) return null;
                                                        ...
                                                        {/* <div className="pt-3 border-t border-white/[0.05] grid grid-cols-2 gap-4 text-xs font-medium">
                                                            <div className="inline-flex items-center gap-2 align-center">
                                                                <p className="text-xs font-semibold uppercase text-white/40 tracking-wider">Verified At</p>
                                                                <p className="text-white/90">{new Date(doc.createdAt).toLocaleString()}</p>
                                                            </div>
                                                        </div> */}
                                                        {/* Metadata Footer */}
                    ...
                    {/* Right Column: Personal Information / Rejection Details */}
                    <div className="space-y-6">
                        <div className="bg-[#141414] border border-white/[0.08] rounded-2xl overflow-hidden h-full">
                            <div className="bg-gradient-to-r from-white/[0.05] to-white/[0.02] p-4 border-b border-white/[0.08] flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-red-500/10 border border-red-500/20 shadow-sm transition-all duration-300">
                                    <div className="w-1.5 h-1.5 rounded-full bg-red-500"></div>
                                    <span className="text-red-500 text-xs font-bold tracking-wide capitalize">{kycData.status}</span>
                                </div>
                            </div>

                            <div className="p-6 space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-1">
                                    {kycData.accountType?.toLowerCase() === 'individual' ? (
                                    ...
                                    <div className="md:col-span-2 space-y-6 pt-4 border-t border-white/[0.08]">
                                        <div className="flex items-center gap-2">
                                            <div className="w-10 h-10 rounded-full bg-white/[0.08] flex items-center justify-center">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-white/50"><path d="M20 10c0 6-8 12-8 12S4 16 4 10a8 8 0 0 1 16 0Z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                                            </div>
                ...
                {allRejections.length > 0 && (
                    <div id="rejection-timeline" className="bg-[#0a0a0a]/80 backdrop-blur-xl border border-white/[0.08] rounded-xl overflow-hidden shadow-2xl mt-8">
                        {/* Header */}
                        <button
                            ...
                            <div className="p-8 px-4 relative animate-in fade-in slide-in-from-top-4 duration-500">
                                <div className="space-y-8 relative">
                                    {allRejections.map((rejection: any, idx: number) => {
                                        const isPrimary = rejection.isPrimary === true;
                                        const recordNum = allRejections.length - idx;

                                        return (
                                            <div key={rejection.id} className="relative flex gap-4 group">
                                                {/* Timeline dot + line */}
                                                ...
                                                {/* Content Card */}
                                                <div className={`flex-1 rounded-2xl border backdrop-blur-md transition-all duration-500 overflow-hidden ${isPrimary
                                                    ? "bg-[#141414] border-white/[0.08] shadow-[0_8px_32px_rgba(255,255,255,0.03)] translate-x-1"
                                                    ...
                                                    <div className="p-6">
                                                        <div className="flex items-start justify-between gap-4 mb-5">
                                                            <div>
                                                                <div className="flex items-center gap-3 mb-1.5">
                                                                    <h4 className={`text-md font-bold tracking-wide ${isPrimary ? "bg-gradient-to-r from-white to-white/70 bg-clip-text text-transparent" : "bg-gradient-to-r from-white to-white/70 bg-clip-text text-transparent group-hover:text-white/50"}`}>
                                                                        Attempt #{recordNum}
                                                                    </h4>
                                                                    {isPrimary && (
                                                                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-gradient-to-r from-white/[0.12] to-white/[0.06] text-white/80 border-white/[0.15] text-xs font-medium border tracking-wider">Current View</span>
                                                                    )}
                                                                </div>
                                                            </div>

                                                            {!isPrimary && (
                                                                <a href={`/admin/kyc/rejected/${encodeId(rejection.id)}`}
                                                        ...
                                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 py-5 border-y border-white/[0.05]">
                                                            ...
                                                            <div className="space-y-2">
                                                                ...
                                                                <p className={`text-[13px] font-semibold tracking-wide pl-[22px] ${isPrimary ? "text-white/90" : "text-white/60"}`}>
                                                            ...
                                                            <p className="text-white/90 text-sm font-medium pl-[22px]">{kycData.rejectedBy || ""}</p>
                                                        </div> */}
                                                        ...
                                                        {rejection.rejectionReason && (
                                                            <div className={`mt-5`}>
                                                                <div className="flex gap-4">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`shrink-0 mt-[5px] ${isPrimary ? "text-white/80" : "text-white/30"}`}>
                                                                        <circle cx="12" cy="12" r="10" /><line x1="12" x2="12" y1="8" y2="12" /><line x1="12" x2="12.01" y1="16" y2="16" />
                                                                    </svg>
                                                                    <p className={`text-[15px] font-medium leading-relaxed tracking-wide ${isPrimary ? "text-white/80" : "text-white/50"}`}>
                                                                        {rejection.rejectionReason}
                                                                    </p>
                                                                </div>
                                                            </div>
...
export default function RejectedKYCDetailsPage() {
    return (
        <AdminDashboardWrapper requireModule="kyc_rejected">
            <RejectedKYCDetailsContent />
